...
void utils::DrawQuadBezier(const Point2f& orgin, const Point2f& end, const Point2f& cPoint1, const Point2f& cPoint2, int nrSegments, float width)
{
	std::vector<Point2f> points{ GetCurve((Vector2f)orgin,(Vector2f)end,(Vector2f)cPoint1,(Vector2f)cPoint2,nrSegments) };
	if (points.size() <= 1) {
		return;
	}

	for (int index{ 1 }; index < points.size(); ++index) {
		DrawLine(points[index - 1], points[index], width);
	}
}
...
bool utils::IsOverlapping(const Point2f& a, const Rectf& r1) {
	return a.x >= r1.left && a.x <= r1.left + r1.width
		&& a.y >= r1.bottom && a.y <= r1.bottom + r1.height;
}
...
bool utils::IsOverlapping(const std::vector<Point2f>& vertices, const Rectf& c)
{
	std::vector<Point2f> rectPoints
	{
		Point2f{c.left,c.bottom},
		Point2f{c.left + c.width,c.bottom},
		Point2f{c.left + c.width, c.bottom + c.height},
		Point2f{c.left, c.bottom + c.height}
	};

	for (size_t i{ 0 }; i < rectPoints.size(); ++i)
	{
		if (utils::IsPointInPolygon(rectPoints[i],vertices))
		{
			return true;
		}
	}
...
std::vector<Point2f> utils::GetCurve(const Vector2f& orgin, const Vector2f& end, const Vector2f& cPoint1, const Vector2f& cPoint2, int nrPoints)
{
	std::vector<Point2f> points;

	const float increment{1.f/nrPoints};
	float time{ 0 };

	while (time <= 1.f) 
	{
		Point2f point{ powf((1 - time),3) * orgin + 3 * powf((1 - time),2) * time * cPoint1 + 3 * (1 - time) * powf(time,2) * cPoint2 + powf(time,3) * end };
		points.push_back(point);
		time += increment;
	}
	return points;
}
Point2f utils::ApplyMatrix(const Transform& trans, const Point2f& pos)
{
	const float scale{ trans.Scale.x };
	const float rotation{ -float(trans.Rotation * M_PI / 180.f) };
	
	Matrix2x3 matrix(
		cosf(rotation) * trans.Scale.x,
		-sinf(rotation) ,
		sinf(rotation) ,
		cosf(rotation) * trans.Scale.y,
		trans.Position.x,
		trans.Position.y);
	return ApplyMatrix(matrix,pos);
}
Point2f utils::ApplyMatrix(const Matrix2x3 matrix, const Point2f& pos)
{
	return matrix.Transform(pos);
}
Vector2f utils::ApplyMatrix(const Transform& trans, const Vector2f& vec)
{
	const float scale{ trans.Scale.x };
	const float rotation{ -float(trans.Rotation * M_PI / 180.f) };
	Matrix2x3 matrix(
	...
	return ApplyMatrix(matrix, vec);
}
Vector2f utils::ApplyMatrix(const Matrix2x3 matrix, const Vector2f& vec)
{
	return matrix.Transform(vec);
}
std::vector<Point2f> utils::ApplyMatrix(const Transform& trans, const std::vector<Point2f>& points)
{
	...
	return ApplyMatrix(matrix, points);
}
std::vector<Point2f> utils::ApplyMatrix(const Matrix2x3 matrix, const std::vector<Point2f>& points)
{
	std::vector<Point2f> newPoints;
	for (const Point2f point : points) {
		newPoints.push_back(ApplyMatrix(matrix, point));
	}
	return newPoints;
}
std::vector<Point2f> utils::GetPolygon(const Rectf& rect)
{
