...
void Texture::Draw(const Point2f& dstBottomLeft,const Color4f color, const Rectf& srcRect) const
{
	const float epsilon{ 0.001f };
	if (!m_CreationOk)
	{
	...
	else
	{
		...
		Draw(dstRect,color, srcRect);
	}
}

void Texture::Draw(const Rectf& dstRect, const Color4f color, const Rectf& srcRect) const
{
	const float epsilon{ 0.001f };
	if (!m_CreationOk)
	{
