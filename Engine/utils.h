...
#pragma region OpenGLDrawFunctionality
	...
	void DrawQuadBezier(
		const Point2f& orgin, 
		const Point2f& end, 
		const Point2f& cPoint1, 
		const Point2f& cPoint2,
		int nrSegments,
		float width = 1.f);
...
#pragma region CollisionFunctionality
	...
	bool IsOverlapping(const Point2f& a, const Rectf& r1);
	...
	bool IsOverlapping(const std::vector<Point2f>& vertices, const Rectf& c);
...
#pragma region Curves
	std::vector<Point2f> GetCurve(const Vector2f& orgin,
		const Vector2f& end,
		const Vector2f& cPoint1,
		const Vector2f& cPoint2,
		int nrPoints);

#pragma endregion

#pragma region transfromation
	
	Point2f ApplyMatrix(const Transform& trans, const Point2f& pos);
	Point2f ApplyMatrix(const Matrix2x3 matrix, const Point2f& pos);

	Vector2f ApplyMatrix(const Transform& trans, const Vector2f& vec);
	Vector2f ApplyMatrix(const Matrix2x3 matrix, const Vector2f& vec);

	std::vector<Point2f> ApplyMatrix(const Transform& trans, const std::vector<Point2f>& points);
	std::vector<Point2f> ApplyMatrix(const Matrix2x3 matrix, const std::vector<Point2f>& points);


	std::vector<Point2f> GetPolygon(const Rectf& rect);
#pragma endregion
}