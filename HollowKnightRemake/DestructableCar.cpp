...
DestructableCar::DestructableCar()
{
	m_Name = "Destructable_Car";
	m_Tag = "destructable";
...
void DestructableCar::Start()
{
	ParticleSystem* particleSystem{ new ParticleSystem
		{
			"VFX/car_particle.png",
			1,
			3,
			3,
			10,
	...
	std::function<void(Particle&)> particleSpawnRule
	{
		[this](Particle& particle)
		{
			...
			Vector2f velocity{cosf(randAngle) * 120,sinf(randAngle) * 20 + 240};
